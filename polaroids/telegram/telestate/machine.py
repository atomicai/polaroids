...
from polaroids.telegram.telestate.constants import KEEP_PREVIOUS
...
class TeleStateMachine(StartupMixin, TeleflaskMixinBase):
    ...
    def register_state(self, name, state=None):
        ...
        return self._register_state(
            name, state=state, allow_setting_defaults=not self.did_init
    ...
    def __repr__(self):
        return "<{clazz}{states!r}>".format(clazz=self.__class__.__name__, states=list(self.states.values()))
