...
class Menu(object, metaclass=ABCMeta):
    ...
    def _activate(
        cls,
        add_history_entry: Union[bool, None] = None,
        update: Union[Update, None, KEEP_PREVIOUS.__class__] = KEEP_PREVIOUS,
    ):
        """
    ...
    def get_value(cls, value: Union[Callable, LambdaType, str, property, JSONType, Any]) -> Any:
        ...
        if is_class_method(value):
            ...
            if "data" in sig.parameters:
                ...
                if (
                    "cls" in sig.parameters
                    or isinstance(first_param.annotation, type)
                    and issubclass(first_param.annotation, Menu)
                ):
    ...
    def _on_callback_query(cls, update: Update):
        ...
        try:
            ...
            bot.answer_callback_query(
                update.callback_query.id, text="Could not find the action you have clicked.", show_alert=True
            )
...
class SelectableMenu(ButtonMenu):
    ...
    def _get_our_buttons(cls, key="selectable_buttons") -> List["Button"]:
        ...
        buttons: List[Union[SelectableButton, CheckboxButton, RadioButton]] = cls.get_value_by_name(key)
        return buttons
...
class CheckboxMenu(SelectableMenu):
    ...
    def process_callback_data(cls, data: CallbackData) -> None:
        ...
        if data.type == cls.MENU_TYPE:
            ...
            # end def
            logger.debug(
                f"Toggling checkbox {button!r} of menu {cls.__name__}, currently having the data"
                f" {cast(MenuData, cls.menu_data).data}."
            )
            cast(MenuData, cls.menu_data).data[button] = not cast(MenuData, cls.menu_data).data[
                button
            ]  # toggle the button.
            cls.refresh(done=False)
            raise AbortProcessingPlease()  # basically a subclass callstack safe "return None"
...
class TextMenu(SendMenu):
    ...
    def on_message_listener(cls, update: Update, msg: Message):
        ...
        if text.startswith("/"):
            ...
            for command, listener in (
                ("done", cls._menu_on_cmd_done),
                ("cancel", cls._menu_on_cmd_cancel),
                ("back", cls._menu_on_cmd_back),
            ):
                # noinspection PyProtectedMember
                for prefix in cast(TeleStateMachineMenuSerialisationAdapter, cls.state_machine).teleflask._yield_commands(
                    command
                ):
