...
class HistoryButton(ChangeMenuButton):
    ...
    @typechecked()
    def __init__(
        self, label: str = "Unnamed Button", delta: int = -1, save: Union[None, bool] = None
    ):  # todo: multi-language for label
        super().__init__(label=label, save=save)
        self.delta = delta
...
class SelectableButton(Button):
    STATE_EMOJIS = {True: "1️⃣", False: "🅾"'}
    title: str
    value: JSONType
    default_selected: bool

    def __init__(self, title, value: JSONType, default_selected: bool = False):
        self.title = title
        self.value = value
...
class CheckboxButton(SelectableButton):
    ...
    def get_selected(self, menu_data: MenuData) -> bool:
        if (
            menu_data.data
            and self.value in menu_data.data
            and isinstance(menu_data.data, dict)
            and isinstance(menu_data.data[self.value], bool)
        ):
...
class RadioButton(SelectableButton):
    ...
    def get_selected(self, menu_data: MenuData) -> bool:
        if menu_data.data and isinstance(menu_data.data, str):
            return menu_data.data == self.value
        # end if
        return self.default_selected
