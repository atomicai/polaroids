...
from polaroids.telegram.telestate import TeleState, TeleStateMachine
...
class MarkForRegister(object):
    ...
    def _build_listener(register_function):
        ...
        def _build_listener_outer_decorator_function(
            *required_keywords: Tuple[Union[Callable, str]]
            ...
            if len(
                required_keywords
            ) == 1 and not isinstance(  # given could be the function, or a single required_keyword.
                required_keywords[0], str
            ):  # not string -> must be function
                ...
                required_keywords = (
                    tuple()
                )  # we call the wrapper ourself, but remove the function from `required_keywords`
...
class TeleMenuMachine(object):
    ...
    def __init__(
        self, states: TeleStateMachineMenuSerialisationAdapter = None, database_driver=None, teleflask_or_tblueprint=None
    ):
    ...
    def register(self, menu_to_register: Type["Menu"]) -> Type["Menu"]:
        ...
        instance_item = TeleMenuInstancesItem(machine=self, state=new_state, menu=menu_to_register)
